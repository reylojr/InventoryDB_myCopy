...
public class App {
	...
	public static void userInterface() {
		...
		String optionsStatement = optionQuestion + optionStatementOne + optionStatementTwo + optionStatementThree + optionStatementFour + optionStatementFive + optionStatementExit;
		try {
			...
			while (!Arrays.asList(userInputOptions).contains(userInput)) {
				userInput = JOptionPane.showInputDialog(JOptionPane.getRootFrame(), intro + optionsStatement).toUpperCase().substring(0, 1);
				...
				switch (userInput) {
					...
					case "3":
						...
						while (userConfirmation.equals("N")) {
							...
							JOptionPane.showMessageDialog(JOptionPane.getRootFrame(), "Current entry for " + userIDInput + " is \n" + tempArray);
							int quantity = Integer.parseInt(JOptionPane.showInputDialog("Current quantity is " + tempArray.getQuantity() + "\n" + " Enter quantity: "));
							double wholeSalePrice = Double.parseDouble(JOptionPane.showInputDialog("Current wholesale price is " + tempArray.getWholesalePrice() + "\n" + "Enter wholesale price: "));
							double salePrice = Double.parseDouble(JOptionPane.showInputDialog("Current sale price is " + tempArray.getSalePrice() + "\n" + "Enter sale price: "));
							String supplierID = JOptionPane.showInputDialog("Current supplier ID is " + tempArray.getSupplierId() + "\n" + "Enter supplier ID: ").toUpperCase();
							JOptionPane.showMessageDialog(JOptionPane.getRootFrame(), "\nYou have chosen to update Supply ID " + userIDInput + " with the follow:\n" + "quantity = " + quantity + "\nwholesale price = " + wholeSalePrice + "\nsale price = " + salePrice + "\nsupplier id = " + supplierID);
							userConfirmation = JOptionPane.showInputDialog("Are these values correct? ").substring(0, 1).toUpperCase();
					...
					case "4":
						...
						while (userConfirmation.equals("N")) {
							...
							userConfirmation = JOptionPane.showInputDialog("Are you sure you want to delete " + userIDInput + "? Yes or No ").toUpperCase().substring(0, 1);
							if (userConfirmation.equals("Y")) {
								testDriver.deleteEntry(userIDInput);
								JOptionPane.showMessageDialog(JOptionPane.getRootFrame(), "Database supply ID " + userIDInput + " has been deleted.");
							} else {
								JOptionPane.showMessageDialog(JOptionPane.getRootFrame(), "Database supply ID " + userIDInput + " was not deleted.");
								break;
							}
						}
						break;
					case "5":
						...
						while (userConfirmation.equals("N")) {
							...
							userConfirmation = JOptionPane.showInputDialog("Inventory ID = " + userIDInput + "\nquantity = " + quantity + "\nwholesale price = " + wholeSalePrice + "\nsale price = " + salePrice + "\nsupplier id = " + supplierID + "\nAre these values correct? ").toUpperCase().substring(0, 1);
							if (userConfirmation.equals("Y")) {
								testDriver.createEntry(userIDInput, quantity, wholeSalePrice, salePrice, supplierID);
								JOptionPane.showMessageDialog(JOptionPane.getRootFrame(), "Created supply ID " + userIDInput + ".\n" + testDriver.searchById(userIDInput));
							} else {
								JOptionPane.showMessageDialog(JOptionPane.getRootFrame(), "Aborted creating database entry for " + userIDInput + ".");
								break;
							}
						}
						break;
